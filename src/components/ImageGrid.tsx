      ...
      {images.map((img) => (
        <div key={`${img.provider}-${img.id}-${img.url}`} className="relative group block">
          ...
          <button
              ...
              className={`absolute top-2 right-2 z-10 bg-white/80 dark:bg-black/60 rounded-full p-2 shadow transition-transform ${isLoading && loadingImageId === String(img.id)
